...
void QuadraticProbing::createAccount(std::string id, int count)
{
    ...
    else
    {
        ...
        {
            if((bankStorage1d[(z+(i*i))%prime2].id == (std:: string)"") or (bankStorage1d[(z+(i*i))%prime2].id == (std:: string)"Deleted")){bankStorage1d[(z+(i*i))%prime2] = b; no_of_accounts++; break;}
            else{ continue;}
        }
...
int QuadraticProbing::getBalance(std::string id) {
    ...
    {
        if((bankStorage1d[(z+(i*i))%prime2].id == (std:: string)"")){return -1;}
        else if(bankStorage1d[(z+(i*i))%prime2].id == (std:: string)"Deleted"){continue;;}
        else if(bankStorage1d[(z+(i*i))%prime2].id == id){return bankStorage1d[(z+(i*i))%prime2].balance;}
        else{continue;}
...
void QuadraticProbing::addTransaction(std::string id, int count) {
    ...
    for(ull i = 0; i < prime2; i++)
    {
        if((bankStorage1d[(z+(i*i))%prime2].id == (std:: string)"")){createAccount(id, count);}
        else if (bankStorage1d[(z+(i*i))%prime2].id == (std:: string)"Deleted"){continue;}
        else
        {
            if(bankStorage1d[(z+(i*i))%prime2].id == id){bankStorage1d[(z+(i*i))%prime2].balance += count; return;}
            else{continue;}
        }
...
bool QuadraticProbing::doesExist(std::string id) {
    ...
    for(ull i = 0; i < prime2; i++)
    {
        if(bankStorage1d[(z+(i*i))%prime2].id == (std:: string)""){return false;}
        else if(bankStorage1d[(z+(i*i))%prime2].id == (std:: string)"Deleted"){continue;}
        else
        {
            if(bankStorage1d[(z+(i*i))%prime2].id == id){return true;}
            else{continue;}
        }
    }
    return false;
...
bool QuadraticProbing::deleteAccount(std::string id) {
    ...
    for(ull i = 0; i < prime2; i++)
    {
        ...
        {
            if(bankStorage1d[(z+i*i)%prime2].id == id)
            {
                bankStorage1d[(z+i*i)%prime2].id = "Deleted";
                bankStorage1d[(z+i*i)%prime2].balance = -2;
                no_of_accounts --;
                return true;
            }
            else{continue;}
